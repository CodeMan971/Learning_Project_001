    ...
    public partial class FrmEmployeeList : Form
    {
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            if (txtName.Text.Trim() != "")
                list = list.Where(x => x.Name.Contains(txtName.Text)).ToList(); // Do NOT have to write whole Name to find it
            if (txtSurname.Text.Trim() != "")
                list = list.Where(x => x.Surname.Contains(txtSurname.Text)).ToList(); // Do NOT have to write whole Surname to find it
            if (cmbDepartment.SelectedIndex != -1)
                list = list.Where(x => x.DepartmentID == Convert.ToInt32(cmbDepartment.SelectedValue)).ToList();
            if (cmbPosition.SelectedIndex != -1)
                list = list.Where(x => x.PositonID == Convert.ToInt32(cmbPosition.SelectedValue)).ToList();
        ...
        private void dataGridView1_RowEnter(object sender, DataGridViewCellEventArgs e)
        {
            ...
            // Only cell numbers are important and are in oreder of properties taken for those columns
            detail.EmployeeID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[0].Value);
            detail.UserNo = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[1].Value);
            detail.Name = dataGridView1.Rows[e.RowIndex].Cells[2].Value.ToString();
            detail.Surname = dataGridView1.Rows[e.RowIndex].Cells[3].Value.ToString();
            //detail.DepartmentName = dataGridView1.Rows[e.RowIndex].Cells[4].Value.ToString(); // Not used
            //detail.PositionName = dataGridView1.Rows[e.RowIndex].Cells[5].Value.ToString(); // Not used
            detail.DepartmentID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[6].Value);
            detail.PositonID = Convert.ToInt32(dataGridView1.Rows[e.RowIndex].Cells[7].Value);
        ...
        {
            DialogResult result = MessageBox.Show("Are you sure to delete tis Employee?", "Warning!", MessageBoxButtons.YesNo);
